...
def mint_nft(nft_contract,tokenId,metadata,owner_address,minter_address):
	...
	#print(nft_contract)
	#print(owner_address)
	#print(minter_address)
	assert isinstance(metadata,dict), f"Error pin_to_ipfs expects a dictionary"	
	jstonStr = json.dumps(metadata)
	files = {
	'file': jstonStr
	}
	headers ={
		"pinata_api_key": "3b067e36311173c5e23e",
		"pinata_secret_api_key": "06978d91b9854dc74847f2846b22239a30cf431bb87a949d5725de92f04a282e"
	}
	response = requests.post('https://ipfs.infura.io:5001/api/v0/add', files=files, auth=('2HvDpBv5yh5EPpVxwd3zZhWEPGR','9da5cd2151c3b0778b35d18f48139c8a'))
	...
	#params = (('arg', CID),)
	#response = requests.post('https://ipfs.infura.io:5001/api/v0/cat', params=params, stream=True)
	#data = requests.get("https://gateway.pinata.cloud/ipfs/" + CID)
	#print(data.text)

	
	if (nft_contract.functions.mint(owner_address, tokenId, tokenURI).call() == True):
