...
def check_txn(order_dict):
		...
		if (platform == 'Algorand'):
			tx = algod_client.search_transaction(txid)
			if (tx.receiver_address == algo_pk & tx.amount ==sell_amount & tx.sender_address==sender):
				return True
			else:
				return False
			...
			if (tx.to == eth_pk and tx.value ==sell_amount and tx.sender_pk==sender):
				return True
			else:
				return False
	...
	for o in orders:
		if ((o.buy_currency == ed_order.sell_currency) and (o.sell_currency == ed_order.buy_currency)):
			if imp_exchange_rate >= o.buy_amount / o.sell_amount:
				...
				tx1 = TX(platform=ed_order.buy_currency, receiver_pk = ed_order.sender_pk, order_id=ed_order.id, tx_id=ed_order.tx_id)
				tx2 = TX(platform=o.buy_currency, receiver_pk = o.sender_pk, order_id=o.id, tx_id=o.tx_id)
				txes.append(tx1)
				txes.append(tx2)
...
def execute_txes(txes):
	...
	tx_ids_eth = send_tokens_eth(w3, eth_sk, eth_txes)
	tx_ids_algo = send_tokens_algo(w3, algo_sk, algo_txes)
...
def trade():
	...
	if request.method == "POST":
		...
		# Your code here
		print("the flask gets upto this point -line 329")
