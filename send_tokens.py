...
def send_tokens_algo(acl, sender_sk, txes):
    ...
    for i, tx in enumerate(txes):
        ...
        order_obj = tx.order
        amount = order_obj.buy_amount

        if order_obj.child != None:
            amount = amount - order_obj.child.buy_amount

        unsigned_tx = transactions.PaymentTxn(sener_pk, params, receiver_pk, amount)
    ...
    for i, tx in enumerate(txes):
        ...
        if order_obj.child != None:
            ...
            tx_dict = {
                ...
                'gas': w3.eth.estimate_gas({'from': sender_pk, 'to': receiver_pk, 'data': b'', 'amount': amount}),
                'to': receiver_pk,
                'value': amount,
                'data': b''}
